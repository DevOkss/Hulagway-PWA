...
const select = (id: string | number | '') => {
  emit('update:modelValue', id as any)
  search.value = ''
  open.value = false
}
  ...
  <div ref="root" class="relative">
    <div class="relative">
      <input
        ...
        class="w-full rounded-xl border border-neutral-200 bg-white px-4 py-2.5 pr-8 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-200"
        autocomplete="off"
        @focus="onFocus"
        @input="open = true"
      />
    ...
    <!-- Selected display outside search input -->
    ...
    <div
      v-if="open"
      class="absolute z-20 mt-1 max-h-56 w-full overflow-auto rounded-xl border border-neutral-200 bg-white py-1 shadow-md"
    >
      <button
        v-if="allowAll"
        type="button"
        class="flex w-full px-4 py-2 text-left text-sm hover:bg-neutral-50"
      ...
      <div v-if="filtered.length === 0" class="px-4 py-2 text-sm text-neutral-400">No barangay found</div>
      <div v-if="!search && filtered.length > 0" class="px-4 py-1.5 text-[11px] text-neutral-400">{{ filtered.length }} barangays — type to filter</div>
    </div>
  </div>
</template>
